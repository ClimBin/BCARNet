...
#     gt = gt > 128
#     pred = np.divide(pred, 255, dtype=np.float64)
#     return pred, gt
...
def _object(pred: np.ndarray, gt: np.ndarray) -> float:
    #print("object",pred.shape,gt.shape)
    try:
        fg = pred * gt
    except:
        print("*********************************fg",)
...
# @numba.jit(nopython=True)
# def _divide_with_xy(pred: np.ndarray, gt: np.ndarray, x, y)-> list[
#     list[Any, Any, Any, Any], list[Any, Any, Any, Any], list[Any, Any, Any, Union[int, Any]]]:
#     h, w = gt.shape
#     area = h * w
...
@numba.njit
def _divide_with_xy(pred: np.ndarray, gt: np.ndarray, x: int, y: int) -> Tuple[
    Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], 
    Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], 
    Tuple[Union[int, Any], Union[int, Any], Union[int, Any], Union[int, Any]]
]:
    h, w = gt.shape
    area = h * w
    ...
    return (gt_LT, gt_RT, gt_LB, gt_RB), (pred_LT, pred_RT, pred_LB, pred_RB), (w1, w2, w3, w4)
